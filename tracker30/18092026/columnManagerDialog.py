...
from ppui.PySide.QtCore import QModelIndex, Qt
from ppui.PySide.QtGui import QDropEvent, QDragEnterEvent
...
class CsvDropArea(QLabel):
    ...
    def setDropHandler(self, handler: Callable[[str], None]):
    ...
    def _hasCsvFile(self, event: QDragEnterEvent) -> bool:
...
class ColumnEditDialog(QDialog):
    CLIPBOARD_FORMAT = 'ppiTracker.columnInfo.v1'
    ...
    def __init__(
        ...
        # Keep the saved values so the manager table can reflect edits after reload.
        ...
        self.nameEdit.setCompleter(self._completer)
        ...
        # Role selector for per-column parameter edit permissions.
        ...
        self.pasteBtn.clicked.connect(self.onPaste)
        QApplication.clipboard().dataChanged.connect(self.updatePasteButtonState)
    ...
    def savedColumn(self) -> Column | None:
        # Used by ColumnListDialog to refresh the edited row with the selected role.
    ...
    def _columnInfoFromDialog(self) -> dict[str, Any]:
        dtypeText = self.typeCombo.currentText()
        dtype = dtypeText.split(' ')[0]
        role = self.roleCombo.currentData() or Role.Supervisor.keyName
        return {
            'format': self.CLIPBOARD_FORMAT,
            'display_name': self.nameEdit.text(),
            'key': self.keyEdit.text(),
            'data_type': dtype,
            'options': [x.strip() for x in self.optionsEdit.text().split(',') if x.strip()],
            'multi_select': self.multiSelectChk.isChecked() and self.multiSelectChk.isEnabled(),
            'visibled': self.visibleChk.isChecked(),
            'is_system': self.systemChk.isChecked(),
            'is_global': self.globalChk.isChecked(),
            'exclude_projects': [
                x.strip() for x in self.excludeEdit.text().split(',') if x.strip()
            ],
            'role': role,
        }

    def onCopy(self):
        data = self._columnInfoFromDialog()
        QApplication.clipboard().setText(json.dumps(data, ensure_ascii=False))
        QMessageBox.information(self, 'Copy Column', 'Column data copied.')

    def _columnInfoFromClipboard(self) -> dict[str, Any] | None:
        try:
            data = json.loads(QApplication.clipboard().text())
        except json.JSONDecodeError:
            return None

        if not isinstance(data, dict) or data.get('format') != self.CLIPBOARD_FORMAT:
            return None
        return data

    def updatePasteButtonState(self):
        if self._column is not None:
            return
        self.pasteBtn.setEnabled(self._columnInfoFromClipboard() is not None)

    def _clearColumnInfoClipboard(self):
        if self._columnInfoFromClipboard() is not None:
            QApplication.clipboard().clear()
        self.updatePasteButtonState()

    def onPaste(self):
        data = self._columnInfoFromClipboard()
        if data is None:
            QMessageBox.warning(self, 'Paste Column', 'Clipboard does not contain column data.')
            self.updatePasteButtonState()
            return

        displayName = str(data.get('display_name') or '')
        self.nameEdit.setText(displayName)
        self.keyEdit.setText(self._generateUniqueKey(displayName, self._existingKeys))

        dtype = str(data.get('data_type') or 'string')
        for i in range(self.typeCombo.count()):
            if self.typeCombo.itemText(i).startswith(dtype):
                self.typeCombo.setCurrentIndex(i)
                break
        ...
        if isinstance(options, list):
            self.optionsEdit.setText(', '.join(str(option) for option in options))
        self.multiSelectChk.setChecked(bool(data.get('multi_select', False)))
        self.visibleChk.setChecked(bool(data.get('visibled', True)))
        self.systemChk.setChecked(bool(data.get('is_system', False)))
        self.globalChk.setChecked(bool(data.get('is_global', False)))

        excludeProjects = data.get('exclude_projects', [])
        if isinstance(excludeProjects, list):
            self.excludeEdit.setText(', '.join(str(project) for project in excludeProjects))

        roleIndex = self.roleCombo.findData(data.get('role') or Role.Supervisor.keyName)
        if roleIndex != -1:
            self.roleCombo.setCurrentIndex(roleIndex)

        self.onSystemToggled(self.systemChk.isChecked())
        self.onGlobalToggled(self.globalChk.isChecked())
        self.onDisplayNameEdited(displayName)
        self.onOptionsChanged()
        self._clearColumnInfoClipboard()
    ...
    def onSave(self):
        ...
        if name and key:
            try:
                savedColumnData: dict[str, Any] = {}
                if self._column is not None and key in self._existingKeys:
                    columnId = self._column.id() or key
                    responseData = self._api.updateColumn(
                        columnId,
                        name,
                        dtype,
                        options,
                    ...
                    if isinstance(responseData, dict):
                        savedColumnData = responseData
                else:
                    responseData = self._api.createColumn(
                        self._root,
                        name,
                        key,
                        dtype,
            ...
            # Build a local copy from the API response and dialog values so the
            # manager can edit a newly-created column without reopening.
            columnData: dict[str, Any] = (
                savedColumnData
                or (self._column.asDict() if self._column else {})
            )
            columnConfig = dict(columnData.get('config', {}))
            columnConfig['options'] = options
            columnConfig['multi_select'] = multiSelect
            columnConfig['role'] = roleObj.keyName
            columnConfig['role_setting'] = roleObj.keyName
            columnData.update(
                {
                    ...
                    'is_system': isSystem,
                    'role': roleObj.keyName,
                    'role_setting': roleObj.keyName,
                    'config': columnConfig,
                    'visibled': visibled,
                }
            )
            self._savedColumn = Column.fromDict(columnData)
            if self._column is None or self._column.roleKey() != roleObj.keyName:
                QMessageBox.information(
                    self,
                    'Column Role Preference',
                    (
                        f'The selected role "{roleObj.displayName}" for "{name}" '
                        'will be saved in project preferences.'
                    ),
                )
            self.accept()
...
class ColumnListDialog(QDialog):
    def __init__(
        ...
        # Show configured role so managers can audit column access rules.
    ...
    def _getColumnNames(self) -> list[str]:
        return [column.displayName() for column in self._columns]
    ...
    def _rebuildColumns(self) -> None:
        ...
        roleOverrides = self._state.columnRoleOverrides(self._root)
        if roleOverrides:
            tempColumns = [
                col.withRole(roleOverrides[col.key()])
                if col.key() in roleOverrides
                else col
    ...
    def _upsertSavedColumn(self, column: Column | None) -> None:
        # Re-apply the saved dialog data after rebuilding from server/template columns.
        if column is None:
            return
        for index, existingColumn in enumerate(self._columns):
            if existingColumn.key() == column.key():
                if not column.id() and existingColumn.id():
                    columnData = column.asDict()
                    columnData['id'] = existingColumn.id()
                    column = Column.fromDict(columnData)
    ...
    def refreshList(self, columns: list[Column] | None = None):
        ...
        for columnOrder in self._columnOrder:
            ...
            # Display role setting from the merged template/database column definition.
    ...
    def addColumn(self):
        ...
        if dlg.exec() == QDialog.DialogCode.Accepted:
            self._isChanged = True
            savedColumn = dlg.savedColumn()
            if savedColumn is not None:
                self._state.setColumnRole(self._root, savedColumn.key(), savedColumn.roleKey())
            self._rebuildColumns()
            self._upsertSavedColumn(savedColumn)
            self.refreshList()
