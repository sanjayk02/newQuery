...
class Column(object):
    ...
    def fromDict(cls, data: dict[str, Any]) -> 'Column':
        # API responses can use different id field names; keep updateColumn on edit path.
        id: str = data.get('id') or data.get('_id') or data.get('column_id') or ''
        ...
        role: str = (
            data.get('role')
            or data.get('role_setting')
            or config.get('role')
            or config.get('role_setting')
            or Role.Supervisor.keyName
        )
