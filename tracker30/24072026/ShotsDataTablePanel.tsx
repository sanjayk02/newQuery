...
const getShotKey = (shot: ShotPivot) =>
    `${shot.group_1}/${shot.group_2}/${shot.group_3}/${shot.relation}`;

const getShotColumnSearchValues = (
    shot: ShotPivot,
    columnName: string,
    ...
    const values: any[] = [];
    const directShot = shot as any;

    Object.keys(directShot || {}).forEach(key => {
        if (normalizeSearchColumn(key) === column) values.push(directShot[key]);
    });

    const aliases: { [key: string]: any[] } = {
        name: [shot.group_1, shot.group_2, shot.group_3],
        name1: [shot.group_1],
        name2: [shot.group_2],
        name3: [shot.group_3],
        group1: [shot.group_1],
        ...
        camdatatype: [camDataTypes[getShotKey(shot)]],
    };

    if (aliases[column]) values.push(...aliases[column]);

    Object.keys(shot.phases || {}).forEach(phase => {
        const phaseLabel = phase.toUpperCase();
        const data = (shot.phases[phase] || {}) as any;
        const mappings: Array<[string, any]> = [
            [`${phaseLabel} APPR`, data.approval_status],
            [`${phaseLabel} APPROVAL`, data.approval_status],
            [`${phaseLabel} APPROVAL STATUS`, data.approval_status],
            [`${phaseLabel} WORK`, data.work_status],
            [`${phaseLabel} WORK STATUS`, data.work_status],
            [`${phaseLabel} TAKE`, data.take],
            [`${phaseLabel} SUBMITTED`, data.submitted_at_utc],
        ];

        mappings.forEach(([label, value]) => {
            if (normalizeSearchColumn(label) === column) values.push(value);
        });

        ((phaseComponents && phaseComponents[phase]) || []).forEach(componentId => {
            if (normalizeSearchColumn(componentId) === column ||
                normalizeSearchColumn(`${componentId} Submitted`) === column) {
                values.push(data.submitted_at_utc);
            }
        });
    });
...
const shotMatchesKeywordSearch = (
    shot: ShotPivot,
    search: ReturnType<typeof parseColumnKeywordSearch>,
    phaseComponents: { [key: string]: string[] },
    camDataTypes: { [shotKey: string]: string },
) => {
    ...
    if (search.targeted) {
        return valuesContainKeyword(
            getShotColumnSearchValues(shot, search.column, phaseComponents, camDataTypes),
            search.keyword,
        );
    }
...
const ShotsDataTablePanel: FC<RouteComponentProps> = () => {
    ...
    const prevShotGroupsRef = useRef(JSON.stringify(filters.shotGroups));
    ...
    const searchOrFilterChanged =
        ...
        prevShotGroupsRef.current !== JSON.stringify(filters.shotGroups);

    const keywordSearch = useMemo(
        () => parseColumnKeywordSearch(searchValue),
        [searchValue],
    );
    ...
    const hasClientSideShotFilter = !!keywordSearch.keyword || filters.shotGroups.length > 0;
    const backendPage    = isGroupView || hasClientSideShotFilter ? 1 : effectivePage + 1;  // API is 1-based
    const backendPerPage = isGroupView || hasClientSideShotFilter ? 10000 : pageProps.rowsPerPage;
    ...
    const keywordFilteredShots = useMemo(() => {
        const list = shots || [];
        const selectedGroups = new Set(
            (filters.shotGroups || []).map(group => group.toLowerCase()),
        );

        return list.filter(shot => {
            if (selectedGroups.size > 0 &&
                !selectedGroups.has((shot.group_1 || '').toLowerCase())) {
                return false;
            }

            return shotMatchesKeywordSearch(shot, keywordSearch, phaseComponents, camDataTypes);
        });
    }, [shots, filters.shotGroups, keywordSearch, phaseComponents, camDataTypes]);

    const sortedAllShots = useMemo(() => {
        if (!isGroupView || !keywordFilteredShots || keywordFilteredShots.length === 0) return keywordFilteredShots || [];
        if (!hasClientSideShotFilter && keywordFilteredShots.length < total) return [];
        ...
        const arr = [...keywordFilteredShots];
        arr.sort((a, b) => {
        ...
        return arr;
    }, [keywordFilteredShots, total, isGroupView, sortKey, sortDir, groupSortDir, hasClientSideShotFilter]);
    ...
    const pagedShots: ShotPivot[] = useMemo(() => {
        if (!isGroupView && !hasClientSideShotFilter) return keywordFilteredShots;

        const source = isGroupView ? sortedAllShots : keywordFilteredShots;
        const start = effectivePage * pageProps.rowsPerPage;
        const end   = start + pageProps.rowsPerPage;
        return source.slice(start, end);
    }, [
        isGroupView,
        hasClientSideShotFilter,
        keywordFilteredShots,
        sortedAllShots,
        effectivePage,
        pageProps.rowsPerPage,
    ]);

    const displayTotal = hasClientSideShotFilter ? keywordFilteredShots.length : total;

    const derivedGroupCounts = useMemo(() => {
        const counts: { [key: string]: number } = {};
        (keywordFilteredShots || []).forEach(shot => {
    ...
    const effectiveGroupCounts =
        Object.keys(groupCounts).length > 0 ? groupCounts : derivedGroupCounts;
    ...
    if (Object.keys(effectiveGroupCounts).length > 0) {
        stableGroupCounts.current = effectiveGroupCounts;
    }
