...
class ApiClient(object):
    ...
    def _raiseForColumnError(self, response: requests.Response, action: str) -> None:
        # Column saves must fail loudly so the edit dialog does not close on rejected updates.
        try:
    ...
    def getColumn(self, root: str, key: str) -> dict[str, Any] | None:
        for column in self.getColumns(root):
            if column.get('key') == key:
                return column
        for column in self.getColumns():
            if column.get('key') != key:
                continue
            if root and column.get('root') not in ('common', root):
                continue
            return column
        return None
    ...
    def createColumn(
        ...
        # Persist required role at top-level and in config for schema compatibility.
        ...
        # Surface duplicate/protected-column errors instead of showing stale table data.
    ...
    def updateColumn(
        ...
        # Keep role_setting in update payload so column permission changes are saved.
        payload: dict[str, Any] = {
            'role': Role.fromKeyName(role).keyName,
            'role_setting': Role.fromKeyName(role).keyName,
            'project': self._projectName,
        ...
        # Surface rejected role/config changes instead of silently keeping the old value.
    ...
    def getHistory(self, entityID: str) -> list[Any]:
        url = f'{self._baseUrl}/api/tracker/entities/{entityID}/history'
        try:
            print(f'[History Debug] GET {url}')
            r = requests.get(
                url,
                headers=self._getHeader(),
                timeout=self.TIMEOUT,
            )
            print(f'[History Debug] Status: {r.status_code}')
            body = r.text
            if len(body) > 4000:
                body = body[:4000] + '... <truncated>'
            print(f'[History Debug] Body: {body}')
            return r.json() if r.status_code == 200 else []
        except Exception as e:
            print(f'[History Debug] Error getting history: {e}')
            return []
