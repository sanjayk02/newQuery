...
const GROUPED_STICKY_THUMBNAIL_WIDTH = 130;
const GROUPED_STICKY_NAME_WIDTH = 180;
/* ____________________________________________________________________________________
    Sticky-column styling for the frozen THUMBNAIL + NAME columns.

    Uses native CSS `position: sticky` (with a `left` offset) instead of a JS scroll
    listener + transform hack. The table already uses `borderCollapse: "separate"`,
    which `position: sticky` requires to work reliably on <td>/<th> elements, and MUI's
    `stickyHeader` already relies on native sticky for the vertical freeze — so this
    composes cleanly for a frozen top-left corner with zero scroll-handler lag, since
    the browser's compositor keeps it in sync every frame instead of a JS round-trip.
  ____________________________________________________________________________________*/
const getGroupedStickyColumnStyle = (
  ...
  if (colId === "group_1_name") {
    return {
      position: "sticky",
      left: GROUPED_STICKY_THUMBNAIL_WIDTH, // sits right after the thumbnail column
      zIndex: header ? 30 : 20,
...
type Column = {
  ...
  kind?: "work" | "appr" | "submitted" | "take" | "assign";
...
/* ____________________________________________________________________________________
  ...
  onFetchAssetComment?: (asset: any, phase: string) => Promise<string>;
};
/* ____________________________________________________________________________________
...
function buildColumns(phaseComponents: { [phase: string]: string[] } | undefined): Column[] {
  return [
    ...
    { id: "dsn_assign_to", label: "DSN Assign to", sortable: true, phase: "dsn" as const, kind: "assign" as const, headerMode: "label" as const },
    ...
    { id: "modeling_assigned_to", label: "MDL Assign to", sortable: true, phase: "mdl" as const, kind: "assign" as const, headerMode: "label" as const },
    ...
    { id: "rigging_assigned_to", label: "RIG Assign to", sortable: true, phase: "rig" as const, kind: "assign" as const, headerMode: "label" as const },
    ...
    { id: "lookdev_assigned_to", label: "LDV Assign to", sortable: true, phase: "ldv" as const, kind: "assign" as const, headerMode: "label" as const },
...
const PhaseStatusText: React.FC<PhaseStatusTextProps> = ({
  asset,
  phase,
  prefix,
  status,
  ...
  const canLoadComment = !!phase && !!onFetchAssetComment && !isEmptyValue(status);
  const tooltipText = loading ? "Loading comment..." : (loadedComment || "No comment found");

  const handleClick = useCallback(async (event: React.MouseEvent<HTMLDivElement>) => {
    if (!canLoadComment || !phase || !onFetchAssetComment) return;

    event.stopPropagation();
    setOpen(true);

    if (loadedComment || loading) return;

    setLoading(true);
    try {
      const text = await onFetchAssetComment(asset, phase);
      setLoadedComment(text || "No comment found");
    } catch (err) {
      console.error("Failed to fetch asset comment:", err);
      setLoadedComment("Failed to load comment");
    } finally {
      setLoading(false);
    }
  }, [asset, canLoadComment, loadedComment, loading, onFetchAssetComment, phase]);
  ...
  const content = (
    <Box
      ...
      lineHeight={1}
      onClick={handleClick}
      style={{ cursor: canLoadComment ? "pointer" : undefined }}
    >
      {prefix && (
        <Typography
          style={{
      ...
      <Typography
        style={{
          ...
          fontStyle: canLoadComment ? "italic" : undefined,
        }}
      >
        {status}
      </Typography>
  ...
  if (!canLoadComment) return content;
...
type ComponentSubmittedTextProps = {
  submittedText: string;
  tooltipText: string;
};

const ComponentSubmittedText: React.FC<ComponentSubmittedTextProps> = ({
  submittedText,
  tooltipText,
}) => {
  const [open, setOpen] = useState(false);
...
function renderAssetField(
  ...
  if (col.isComponent) {
    ...
    let tooltipSubmittedText = "—";
    if (submittedAt) {
      try {
        tooltipSubmittedText = dateTimeFormat.format(new Date(submittedAt));
      } catch {
        tooltipSubmittedText = "—";
      }
    }
    const tooltipText = componentData
      ? [
        `Component: ${componentData.component || col.id}`,
        `Phase: ${col.phase || "—"}`,
        `Submitted: ${tooltipSubmittedText}`,
      ].join("\n")
      : "No submission available";

    return (
      <ComponentSubmittedText
  ...
  if (col.kind === "appr" || col.kind === "work") {
    ...
    return (
      <PhaseStatusText
        ...
        prefix={prefix}
        status={status}
        onFetchAssetComment={onFetchAssetComment}
      />
    );
  }
  {/* Phase columns (WORK/APPR/SUBMITTED/TAKE) */}
...
function buildCellStyle(
  ...
  if (header && col.id !== "thumbnail" && col.id !== "group_1_name") {
...
/* ____________________________________________________________________________________
  ...
  ____________________________________________________________________________________*/
/* ──────────────────────────────────────────────────────────────────────────
  Per-row data component for the grouped table.
  Owns its own hover state so the row gets a light-blue highlight on mouse
  enter, and persists that highlight (same color) when clicked/selected.
  ────────────────────────────────────────────────────────────────────────── */
type AssetGroupedDataRowProps = {
  asset: any;
...
const AssetGroupedDataRow: React.FC<AssetGroupedDataRowProps> = ({
  ...
  selectedRowId, onRowSelect, onFetchAssetComment,
}) => {
  ...
  // Hover state — gives the row a light-blue highlight on mouse over.
  const [isHovered, setIsHovered] = useState(false);
  // Highlight colors. Selected wins over hover when both are true.
  const SELECTED_BG = 'rgba(173, 216, 230, 0.4)'; // light blue — click/selected color
    ...
    >
      {visibleColumns.map((col) => {
        ...
        return (
          <TableCell
            key={col.id}
            align={col.id === "thumbnail" ? "left" : "center"}
            style={rowBg
              ? { ...baseStyle, backgroundColor: rowBg }
              : baseStyle}
          >
            {renderAssetField(asset, col, dateTimeFormat, latestComponents, thumbnails, onFetchAssetComment)}
          </TableCell>
        );
      })}
    </TableRow>
...
const AssetsGroupedDataTable: React.FC<Props> = ({
  ...
  const isGroupedColHidden = useCallback(
    (groupColId: string, col?: Column) => {
      ...
      if (col && col.kind === "assign") return hiddenColumns.has(groupColId);
      ...
      <Box className={classes.scroller}>
        <Table size="small" stickyHeader className={classes.table}>
          ...
          <colgroup>
            {visibleColumns.map((col) => {
              if (col.id === "thumbnail") {
                return <col key={col.id} style={{ width: GROUPED_STICKY_THUMBNAIL_WIDTH }} />;
              }
              if (col.id === "group_1_name") {
                return <col key={col.id} style={{ width: GROUPED_STICKY_NAME_WIDTH }} />;
              }
              if (col.id === "relation") {
                return <col key={col.id} style={{ width: UI.RELATION_WIDTH }} />;
              }
              return <col key={col.id} style={{ width: UI.PHASE_COL_WIDTH }} />;
            })}
          </colgroup>
          <TableHead>
            <TableRow>
                      ...
                      return (
                        ...
                        >
                          {shouldShowGroupLabel ? (
                            <Box display="flex" alignItems="center" style={{ paddingLeft: isName ? 8 : 0 }}>
                              <IconButton
                                size="small"
