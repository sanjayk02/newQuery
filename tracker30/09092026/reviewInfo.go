/* ──────────────────────────────────────────────────────────────────────────
	...
	* - 03-06-2026 - Added review-queue shot functions (CountReviewShots, ListReviewShots, ListReviewShotsPivot).
	...
	* - CountReviewShots: Counts unique review-queue shot groups (check status).
	* - ListReviewShots: Lists paged latest per-phase review-queue shot rows.
	* - ListReviewShotsPivot: Lists review-queue shots pivoted into ShotPivot.
	* - ListReviewShotPhases: Returns the distinct review-queue phases (dynamic phase set).
...
// uniqueStrings removes duplicate values while preserving their order.
...
// appendRequiredReviewInfoColumns adds the columns required for joins and
// ordering without duplicating existing columns.
func appendRequiredReviewInfoColumns(cols []string) []string {
	required := []string{
		"project", 
		"take_path", 
		"deleted", 
	...
	result := make([]string, 0, len(cols)+len(required))
	seen := make(map[string]struct{}, len(cols)+len(required))
...
// quoteReviewInfoColumns quotes review information column names for use in SQL statements.
func quoteReviewInfoColumns(cols []string) []string {
...
// qualifyReviewInfoColumns prefixes each column with the specified table
// alias and quotes the column name.
func qualifyReviewInfoColumns(cols []string, tableAlias string) []string {
	qualified := quoteReviewInfoColumns(cols)

	for i := range qualified {
		qualified[i] = tableAlias + "." + qualified[i]
	}

	return qualified
}

// buildGroupSetsFilter builds a SQL condition for multiple Group paths.
//
// Conditions within each Group path are combined with AND, while the paths
// themselves are combined with OR.
func buildGroupSetsFilter(groupSets [][]string) (string, []interface{}) {
	clauses := make([]string, 0, len(groupSets))
	args := make([]interface{}, 0)

	for _, groupSet := range groupSets {
		parts := make([]string, 0, len(groupSet)*2)

		for i, group := range groupSet {
			// Match both the generated Group column and the corresponding JSON value.
			parts = append(
				parts, 
...
// ListFlexible retrieves paginated review information using selectable
// fields and multiple Group paths.
func (r *ReviewInfo) ListFlexible(
	db *gorm.DB,
	params *entity.ListReviewInfoParamsFlexible,
) ([]*entity.ReviewInfo, []*entity.ShotProperty, int, error) {
	base := params.ListReviewInfoParams
	selectedColumns := uniqueStrings(params.Fields)
	selectAllReviewColumns := len(selectedColumns) == 0

	stmtA := db.Model(&model.ReviewInfo{})

	if selectAllReviewColumns {
		stmtA = stmtA.Select("*")
	} else {
		innerColumns := appendRequiredReviewInfoColumns(selectedColumns)
		stmtA = stmtA.Select(
			strings.Join(quoteReviewInfoColumns(innerColumns), ", "),
		)
	}

	groupSets := make([][]string, 0, len(params.GroupSets)+1)
	if len(base.Group) > 0 {
		groupSets = append(groupSets, base.Group)
	}
	groupSets = append(groupSets, params.GroupSets...)
	if clause, args := buildGroupSetsFilter(groupSets); clause != "" {
		stmtA = stmtA.Where(clause, args...)
	}

	stmtA = stmtA.Where("project = ?", base.Project)
	if base.Studio != nil {
		stmtA = stmtA.Where("studio = ?", *base.Studio)
	}
	if base.TaskID != nil {
		stmtA = stmtA.Where("task_id = ?", *base.TaskID)
	}
	if base.SubtaskID != nil {
		stmtA = stmtA.Where("subtask_id = ?", *base.SubtaskID)
	}
	if base.Root != nil {
		stmtA = stmtA.Where("root = ?", *base.Root)
	}
	if base.Relation != nil {
		stmtA = stmtA.Where("relation IN (?)", base.Relation)
	}
	if base.Phase != nil {
		stmtA = stmtA.Where("phase IN (?)", base.Phase)
	}
	if base.Component != nil {
		stmtA = stmtA.Where("component = ?", *base.Component)
	}
	if base.Take != nil {
		stmtA = stmtA.Where("take = ?", *base.Take)
	}
	...
	showDeleted := false
	if base.ModifiedSince != nil {
		stmtA = stmtA.Where("modified_at_utc >= ?", *base.ModifiedSince)
		order = "a.modified_at_utc asc"
		showDeleted = true
	} else {
		stmtA = stmtA.Where("deleted = ?", 0)
	...
	joinSelect := "a.*, b.*"
	if !selectAllReviewColumns {
		outerColumns := qualifyReviewInfoColumns(selectedColumns, "a")
		joinSelect = strings.Join(append(outerColumns, "b.*"), ", ")
	}
...
/*
	 ──────────────────────────────────────────────────────────────────────────
		ReviewShotStatuses defines the set of statuses considered for filtering review shots.
		Currently, it includes only the "check" status, but can be expanded as needed.
...
/*
	 ──────────────────────────────────────────────────────────────────────────
		reviewShotPhaseRow represents a single row of review shot phase data, including identifying information
		(project, root, group1-3, relation, phase) and status information (work status, approval status, submission time, and take).
...
/*
	 ──────────────────────────────────────────────────────────────────────────
		buildReviewShotPhaseIn constructs a SQL WHERE clause segment that filters rows based on the provided
		phases. If no phases are provided, it defaults to the predefined ReviewShotPhases.
		It returns the SQL fragment (e.g., "phase IN (?, ?, ?)") and a slice of arguments corresponding to the phases.
...
/*
	 ───────────────────────────────────────────────────────────────────────────
		buildReviewShotStatusWhere constructs a SQL WHERE clause segment that filters rows based on the provided
		statuses. If no statuses are provided, it leaves the result unfiltered.
		It returns the SQL fragment (e.g., "(work_status IN (?, ?) OR approval_status IN (?, ?))")
		and a slice of arguments corresponding to the statuses for both work_status and approval_status.
...
/*
	──────────────────────────────────────────────────────────────────────────
	buildReviewShotSort constructs the SQL ORDER BY clause for review shots based on the provided orderKey and direction.
	It returns the sort selection (if any), sort arguments, and the group order string for stable sorting.
...
──────────────────────────────────────────────────────────────────────────
	...
	- statuses: Status set applied to work_status OR approval_status; empty means all statuses.
...
WITH unique_groups AS (
    ...
    FROM (
        ...
        LEFT JOIN (
            SELECT project, root, group_1, group_2, group_3, relation, phase,
                   work_status, approval_status, modified_at_utc
            FROM t_review_info
            WHERE project = ? AND root = 'shots' AND deleted = 0
              AND ` + phaseClause + `
        ) b
...
func (r *ReviewInfo) ListReviewShots(
	...
	// Pick the latest row per phase before applying the status filter. Applying
	// the status filter first can display an older take after a newer approval.
	query := `
...
grp_agg AS (
    ...
    WHERE EXISTS (
        ...
        WHERE mg.project = latest_reviews_phase_sort.project
          AND mg.root = latest_reviews_phase_sort.root
          AND mg.group_1 = latest_reviews_phase_sort.group_1
          AND mg.group_2 <=> latest_reviews_phase_sort.group_2
          AND mg.group_3 <=> latest_reviews_phase_sort.group_3
          AND mg.relation = latest_reviews_phase_sort.relation
    )
    GROUP BY project, root, group_1, group_2, group_3, relation
),
unique_groups AS (
...
──────────────────────────────────────────────────────────────────────────
	...
	ReviewShotGroupCount represents the count of distinct review-queue shots
	under a specific (group_1, group_2) combination. It is used to provide
	aggregate information about the number of shots per group, which can be
	leveraged for pagination and filtering in the client application.

──────────────────────────────────────────────────────────────────────────
*/
type ReviewShotGroupCount struct {
	Group1    string `gorm:"column:group_1" json:"group_1"`
	Group2    string `gorm:"column:group_2" json:"group_2"`
	ShotCount int    `gorm:"column:shot_count" json:"shot_count"`
}

/*
──────────────────────────────────────────────────────────────────────────

	ListReviewShotGroupCounts returns, per (group_1, group_2), the number of
	distinct review-queue shots (group_1, group_2, group_3, relation) under it.
	From this the client can derive:
	  - per group_1: total shots and number of group_2s,
	  - per group_1/group_2: shot count,
	  - the group_1 filter-menu counts,
	without fetching any shot rows. This lets group view paginate per page
	instead of loading the entire data set.

	Parameters mirror the pivot: project (required), phases (defaults to the
	dynamic set when empty), statuses (empty means all statuses).

──────────────────────────────────────────────────────────────────────────
*/
func (r *ReviewInfo) ListReviewShotGroupCounts(
	ctx context.Context,
	project string,
	phases []string,
	statuses []string,
) ([]ReviewShotGroupCount, error) {
	if project == "" {
		return nil, fmt.Errorf("project is required")
	}

	// Resolve the phase set dynamically when none supplied, matching the pivot.
	if len(phases) == 0 {
		dyn, err := r.ListReviewShotPhases(ctx, project, statuses)
		if err != nil {
			return nil, err
...
SELECT group_1, COALESCE(group_2, '') AS group_2, COUNT(*) AS shot_count
FROM groups
...
`
	...
	var rows []ReviewShotGroupCount
	if err := r.db.WithContext(ctx).Raw(query, args...).Scan(&rows).Error; err != nil {
		return nil, fmt.Errorf("ListReviewShotGroupCounts: %w", err)
	}
	return rows, nil
}
