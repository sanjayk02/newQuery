...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(
        ...
        def _mark(label: str) -> None:
            print(f'[TIMING][MainWindow] {label}: {time.perf_counter() - _t0:.3f}s elapsed')

        super().__init__(parent)
        _mark('super().__init__ done')
        self._appInfo = appInfo
        self._state = state
        self._pipelineSetting = state.pipelineSetting()
        self._service = Service(self._pipelineSetting)
        _mark('Service() constructed')
        self._isDev = isDev
        ...
        _mark('ApiClient() constructed')
        self._dataRepo = DataRepository(self._isDev)
        _mark('DataRepository() constructed')
        ...
        _mark('TableWidget/TreeWidget constructed')
