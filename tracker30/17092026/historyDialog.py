...
class HistoryDialog(QDialog):
    def __init__(
        ...
        self._rollbackResult: tuple[str, dict[str, Any]] | None = None
    ...
    def rollbackResult(self) -> tuple[str, dict[str, Any]] | None:
        return self._rollbackResult
    ...
    def _buildHistoryBlock(
        ...
        for change in self._historyChanges(history):
            ...
            grid.addWidget(columnLabel, row, 0)
            grid.addWidget(valueLabel, row, 1)
            grid.addWidget(rollbackButton, row, 2)
            row += 1
        ...
        allRollbackButton.clicked.connect(
            lambda checked=False, eid=entityID, h=history: self.doRollback(
                eid,
                self._historyID(h),
                '',
                h,
            ...
            try:
                if columnName and history is not None:
                    ...
                    if not root or not group:
                        raise ValueError(
                            'History data does not contain root/group for column rollback.'
                        )
                    changeData = {columnName: value}
                    self._api.updateEntity(entityID, root, group, changeData)
                    self._rollbackResult = (group, changeData)
                else:
                    if history is None:
                        raise ValueError('History data is not available for rollback.')
                    group = self._historyGroup(history)
                    snapshotData = self._snapshotData(history)
                    if not group:
                        raise ValueError('History data does not contain group for rollback.')
                    self._api.rollback(entityID, historyID)
                    self._rollbackResult = (group, snapshotData)
