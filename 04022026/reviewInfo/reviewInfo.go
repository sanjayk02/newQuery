...
func (r *ReviewInfo) List(
	...
	if params == nil {
		return nil, 0, errors.New("params cannot be nil")
	}
	...
	for i, g := range params.Group {
		stmt = stmt.Where(fmt.Sprintf("JSON_UNQUOTE(JSON_EXTRACT(`groups`, '$[%d]')) = ?", i), g)
	}
	if params.Relation != nil && len(params.Relation) > 0 {
		stmt = stmt.Where("relation IN (?)", params.Relation)
	}
	if params.Phase != nil && len(params.Phase) > 0 {
	...
	// Determine ordering and deletion filter
	order := "`id` DESC"
...
func (r *ReviewInfo) Update(
	...
	if err := tx.Where("`deleted` = ?", 0).
		...
		return nil, fmt.Errorf("failed to find review for update: %w", err)
	}
...
func (r *ReviewInfo) Delete(
	...
	if err := tx.Where("`deleted` = ?", 0).
		...
		return fmt.Errorf("failed to find review for deletion: %w", err)
	}
...
func (r *ReviewInfo) ListAssets(
	...
	stmt = stmt.Order("group_1").Order("relation")

	var reviews []*model.ReviewInfo
	perPage := params.GetPerPage()
	offset := perPage * (params.GetPage() - 1)
	if err := stmt.Select("project", "root", "group_1", "relation").
		Limit(perPage).
		Offset(offset).
...
func (r *ReviewInfo) ListAssetReviewInfos(
	...
	stmt := db.Select("b.*").
		Table("(?) AS a", stmtA).
		Joins("LEFT OUTER JOIN (?) AS b ON a.project = b.project AND a.root = b.root AND a.group_1 = b.group_1 AND a.relation = b.relation AND a.phase = b.phase AND a.modified_at_utc = b.modified_at_utc", stmtB)

	var reviews []*model.ReviewInfo
	if err := stmt.Scan(&reviews).Error; err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, fmt.Errorf("failed to fetch asset review infos: %w", err)
	}
...
func (r *ReviewInfo) ListShotReviewInfos(
	...
	if len(params.Groups) < 3 {
		return nil, errors.New("groups must have at least 3 elements")
	}
	...
	stmtA := db.Select(
		...
		Where("project = ?", params.Project).
		Where("root = ?", "shots").
		Where("group_1 = ?", params.Groups[0]).
		Where("group_2 = ?", params.Groups[1]).
		Where("group_3 = ?", params.Groups[2]).
		Where("relation = ?", params.Relation).
		Where("deleted = ?", 0).
		Group("project").
		Group("root").
	...
	stmt := db.Select("b.*").
		Table("(?) AS a", stmtA).
		Joins("LEFT OUTER JOIN (?) AS b ON a.project = b.project AND a.root = b.root AND a.group_1 = b.group_1 AND a.group_2 = b.group_2 AND a.group_3 = b.group_3 AND a.relation = b.relation AND a.phase = b.phase AND a.modified_at_utc = b.modified_at_utc", stmtB)

	var reviews []*model.ReviewInfo
	if err := stmt.Scan(&reviews).Error; err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, fmt.Errorf("failed to fetch shot review infos: %w", err)
	}
