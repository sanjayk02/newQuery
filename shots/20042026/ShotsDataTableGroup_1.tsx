...
const ShotDataRow: React.FC<{
...
}> = ({ shot, thumbnails, dateTimeFormat, columnsState, isLastRow, phaseComponents, latestComponents, selectedRowId, onRowSelect }) => {
    const thumbKey = `${shot.group_1}-${shot.group_2}-${shot.group_3}-${shot.relation}`;
    const compKey  = `${shot.group_1}-${shot.group_2}-${shot.group_3}-${shot.relation}`;
    // Stable identity for row selection (includes root/project/component to avoid collisions)
    ...
    return (
        <TableRow
            hover
            onDoubleClick={() => onRowSelect && onRowSelect(rowId)}
            style={{
