...
func (r *ReviewInfo) CountReviewShots(
	...
	// (shot, phase) across ALL phases, then keep shots whose latest row matches
	// the status in at least one phase. Status is evaluated on the latest review
	// (not on any historical row). No phase scope — the queue counts every shot
	// currently in the status regardless of phase. `phases` is accepted for
	// signature compatibility but is not used as a filter.
	query := `
WITH latest_phase AS (
...
func (r *ReviewInfo) ListReviewShots(
	...
	// evaluated on the current review; no phase scope. `phases` is accepted for
	// signature compatibility but is not used as a filter.
	query := `
WITH latest_phase AS (
...
// ReviewShotGroupCount is one (group_1, group_2) bucket with its shot count,
// used to build the group-view tree totals server-side (so the client no longer
// needs to load every shot just to compute "DRK101 (4/13)" style totals).
...
──────────────────────────────────────────────────────────────────────────
	...
	dynamic set when empty), statuses (defaults to ReviewShotStatuses).
