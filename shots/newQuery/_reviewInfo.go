...
latest_reviews_sort AS (
    SELECT *, ROW_NUMBER() OVER (ORDER BY group_1 ` + dir + `) AS _sort_order
    FROM latest_reviews
),
...
unique_groups AS (
    SELECT project, root, group_1, group_2, group_3, relation,
           MIN(_phase_order) AS _min_phase_order
    FROM latest_reviews_phase_sort
    GROUP BY project, root, group_1, group_2, group_3, relation
    ORDER BY group_1 IS NULL, group_1 ` + dir + `, group_2 ASC, _min_phase_order
    LIMIT ? OFFSET ?
),
...
ORDER BY group_1 IS NULL, group_1 ` + dir + `, group_2 ASC, group_3 ASC, relation ASC, _phase_order ASC;
`
...
func (r *ReviewInfo) ListReviewShotsPivot(
	...
	rows, err := r.ListReviewShots(ctx, project, phases, statuses, direction, limit, offset)
	if err != nil {
		return nil, 0, err
	}
	if len(rows) == 0 {
