...
const ASSIGNMENT_COLUMNS_BY_PHASE: { [key: string]: { id: keyof Asset; label: string } } = {
  dsn: { id: "dsn_assign_to", label: "DSN Assign to" },
  mdl: { id: "modeling_assigned_to", label: "MDL Assign to" },
  rig: { id: "rigging_assigned_to", label: "RIG Assign to" },
  ldv: { id: "lookdev_assigned_to", label: "LDV Assign to" },
};
...
const WORK_STATUS: { [key: string]: Status } = {
  ...
  svApproved: { displayName: "SV Approved", color: "#00ff4c" },
...
/* ──────────────────────────────────────────────────────────────────────────
  Builds the dynamic component columns for a given phase based on the project's
  pipeline configuration. Each component column will have a label like "COMPONENT Submitted"
  and will be sortable by the component's submission date.
  ───────────────────────────────────────────────────────────────────────── */
...
/* ──────────────────────────────────────────────────────────────────────────
  Builds the complete list of columns for the assets data table, including fixed columns (Thumbnail, Name)
  and dynamic columns for each pipeline phase based on the project's configuration. Dynamic component columns
  are generated for each phase using the buildComponentColumns function.
  ───────────────────────────────────────────────────────────────────────── */
function buildColumns(phaseComponents: { [phase: string]: string[] } | undefined): Column[] {
  return [
    ...
    { id: "dsn_assign_to",       label: "DSN Assign to", colors: ASSET_PHASES.dsn, sortable: true, sortKey: "dsn_assign_to" },
    ...
    { id: "modeling_assigned_to", label: "MDL Assign to", colors: ASSET_PHASES.mdl, sortable: true, sortKey: "modeling_assigned_to" },
    ...
    { id: "rigging_assigned_to", label: "RIG Assign to", colors: ASSET_PHASES.rig, sortable: true, sortKey: "rigging_assigned_to" },
    ...
    { id: "lookdev_assigned_to", label: "LDV Assign to", colors: ASSET_PHASES.ldv, sortable: true, sortKey: "lookdev_assigned_to" },
...
const STICKY_THUMBNAIL_WIDTH = 140;
const STICKY_NAME_WIDTH = 220;

const getStickyAssetColumnStyle = (
  columnId: string,
  options: { header?: boolean; backgroundColor?: string } = {},
): React.CSSProperties => {
  const { header = false, backgroundColor = "#3f3f3f" } = options;
  ...
  if (columnId === "thumbnail") {
    return {
      ...
      left: 0,
      zIndex: header ? 6 : 3,
      width: STICKY_THUMBNAIL_WIDTH,
      minWidth: STICKY_THUMBNAIL_WIDTH,
      maxWidth: STICKY_THUMBNAIL_WIDTH,
      backgroundColor,
    };
  }

  if (columnId === "group_1_name") {
    return {
      position: "sticky",
      left: STICKY_THUMBNAIL_WIDTH,
      zIndex: header ? 6 : 3,
...
/* ──────────────────────────────────────────────────────────────────────────
  Utility function to determine if only fixed columns are visible based on the current column configuration and hidden columns set.
  This is used to toggle compact mode in the table when all non-fixed columns are hidden.
  ───────────────────────────────────────────────────────────────────────── */
...
/* ──────────────────────────────────────────────────────────────────────────
  Utility function to determine if a value should be considered empty for display purposes.
  This treats null, undefined, empty strings, and certain placeholder values as empty.
  ───────────────────────────────────────────────────────────────────────── */
...
/* ──────────────────────────────────────────────────────────────────────────
  Utility function to format a value for display in the table. It checks 
  if the value is empty using the isEmptyValue function,
  and returns a placeholder if it is, or the trimmed string representation 
  of the value if it's not.
  ───────────────────────────────────────────────────────────────────────── */
...
const getAssetValue = (asset: Asset, key: string): any => {
  return (asset as Record<string, any>)[key];
};

/* ──────────────────────────────────────────────────────────────────────────
  A custom table cell component that supports displaying a tooltip with 
  multi-line text when clicked. It also applies conditional styling based 
  on the provided status and border styles.
  The cell displays either the provided children or a default text based on
  the status, and shows the tooltip if there is tooltip text available.
  ───────────────────────────────────────────────────────────────────────── */
...
/* ──────────────────────────────────────────────────────────────────────────
  Component that renders the table header for the assets data table. 
  It displays column labels and handles sorting interactions.
  Each column can be styled based on its configuration, and sortable columns 
  will show a sort indicator when active.
  The component receives the current sort key and direction to determine how 
  to display the sort state for each column.
  ───────────────────────────────────────────────────────────────────────── */
          ...
          return (
            <TableCell
              ...
              style={{
                ...
                ...getStickyAssetColumnStyle(column.id, {
                  header: true,
                  backgroundColor: column.colors ? column.colors.backgroundColor : "#3f3f3f",
                }),
...
/* ──────────────────────────────────────────────────────────────────────────
  Component that renders a single row in the assets data table for a given asset.
  It displays the asset's thumbnail, name, and status for each phase and component.
  The component uses helper functions to extract and format the relevant data for each cell,
  and applies conditional styling based on the asset's status and the current column configuration.
  It also handles the display of tooltips for phase and component cells, showing detailed information when available.
  The component receives the asset data, thumbnails, date formatting function, 
  and configuration for which columns are hidden and which components belong to each phase.
  ───────────────────────────────────────────────────────────────────────── */
const AssetRow: React.FC<AssetRowProps & {
  ...
  // Hover state — gives the row a highlight on mouse over.
  const [isHovered, setIsHovered] = React.useState(false);

  // Highlight colors. Selected wins over hover when both are true.
  const SELECTED_BG = 'rgba(0, 191, 255, 0.79)';  // click/selected color
  const HOVER_BG    = 'rgba(173, 216, 230, 0.4)';  // change THIS for the hover color
  const rowBg       = isRowSelected ? SELECTED_BG : (isHovered ? HOVER_BG : undefined);
  const stickyBg    = rowBg || "#3f3f3f";
  ...
  /* ──────────────────────────────────────────────────────────────────────────
    Helper function to get the display text for a component cell. 
    It retrieves the relevant data for the component and formats the submission date for display. 
    If no data is available, it returns a placeholder.
    ───────────────────────────────────────────────────────────────────────── */
  ...
  /* ──────────────────────────────────────────────────────────────────────────
    Helper function to get the tooltip text for a component cell. 
    It retrieves the relevant data for the component and formats the submission date for display. 
    If no data is available, it returns a placeholder.
    ───────────────────────────────────────────────────────────────────────── */
  ...
  /* ──────────────────────────────────────────────────────────────────────────
    Helper function to extract the work status, approval status, 
      and take number for a given phase from the asset data.
    It constructs the relevant keys based on the phase name, 
    retrieves the values, and maps them to display statuses.
    It also formats the take number for display, returning a placeholder if it's empty.
    ───────────────────────────────────────────────────────────────────────── */
  ...
  return (
    ...
    >
      {!isHidden("thumbnail") && (
        <TableCell
          style={{
            ...getStickyAssetColumnStyle("thumbnail", { backgroundColor: stickyBg }),
          }}
        >
      ...
      {!isHidden("group_1_name") && (
        <TableCell
          style={{
            ...getStickyAssetColumnStyle("group_1_name", { backgroundColor: stickyBg }),
          }}
        >
      ...
      {(Object.entries(ASSET_PHASES) as Array<[string, { lineColor: string }]>).map(
          ([phase, { lineColor }]) => {
            const ids = {
              ...
              take: `${phase}_take`,
              assign: (ASSIGNMENT_COLUMNS_BY_PHASE[phase] || {}).id,
            };
            ...
            return (
              <React.Fragment key={`${asset.group_1}-${asset.relation}-${phase}`}>
                ...
                {/* ASSIGN TO */}
                {ids.assign && !isHidden(ids.assign) && (
                  <TableCell
                    style={{
                      textAlign: "center",
                      verticalAlign: "middle",
                      padding: "8px",
                      borderLeft: firstId === ids.assign ? rail : "none",
                      borderRight: lastId === ids.assign ? rail : "none",
                      borderBottom: isLastRow ? rail : "none",
                    }}
                  >
                    {formatForDisplay(getAssetValue(asset, ids.assign))}
                  </TableCell>
                )}
...
/* ──────────────────────────────────────────────────────────────────────────
  Main component that renders the assets data table for a given project. 
  It fetches asset thumbnails, builds the column configuration based on the project's pipeline phases,
  and renders the table header and body with the appropriate data and styling.
  The component supports sorting, column visibility toggling, and displays detailed information 
  for each asset and its phases/components.
  It receives the project data, list of assets, table footer, date formatting function, 
  sorting handlers, hidden columns configuration, and phase components configuration as props.
  If no project is provided, it returns null to avoid rendering an empty table.
  ───────────────────────────────────────────────────────────────────────── */
