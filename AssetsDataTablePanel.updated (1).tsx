...
const AssetsDataTablePanel: React.FC<RouteComponentProps> = () => {
  ...
  // Determine if grouped (grid) view is active based on barView state
  const isGrouped = barView === 'grid';
  // Backend view mode: grid => grouped, list => list
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <TextField
                ...
                onChange={(e) => {
                  ...
                  setBarSearch(v);
                  // optionally connect this to your existing table filter:
                  // handleFilterAssetNameChange({ target: { value: v } } as any);
                }}
                InputProps={{
                  style: {
                    height: 28,
      ...
      <StyledTableDiv>
        <StyledPaper>
          <StyledContentDiv>
            {/*
              IMPORTANT:
              - list view   => render the main table (AssetsDataTable) and use view=list
              - grouped view => render the collapsible top-node list (AssetsGroupedDataTable) and use view=grouped
            */}
            {!isGrouped ? (
              <AssetsDataTable
