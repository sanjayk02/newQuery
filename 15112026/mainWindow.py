...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def _beginStagedInitialLoad(self) -> None:
        ...
        # Load only the visible tab on startup. The other tab loads the first
        # time the user opens it, keeping the app responsive after launch.
        self._loadRootStaged(self._currentRoot, nextRoot=None)
        ...
        if checked:
            ...
            if root != self._currentRoot:
                ...
                if root not in self._loadedRoots:
                    ...
                    self._loadRootStaged(root, nextRoot=None)
...
class DataWidget(QWidget):
    ...
    def showCellContextMenu(self, view: QAbstractItemView, pos: QPoint) -> None:
        ...
        historyItems = self._historyItemsForSelection(clickedGroup, selectedGroups)

        menu = QMenu(view)
        editAction = menu.addAction('Edit in Cell')
        editAction.setEnabled(
            not isMultiSelectedClick and self.canDirectEditCell(index, showWarning=False)
        )
        historyAction = menu.addAction('History')
        historyAction.setEnabled(bool(historyItems))

        selectedAction = menu.exec(view.viewport().mapToGlobal(pos))
        if selectedAction == editAction:
            self.beginDirectCellEdit(index)
        elif selectedAction == historyAction:
            self.showHistoryDialog(historyItems)

    def _historyItemsForSelection(
        self,
        clickedGroup: str | None,
        selectedGroups: list[str],
    ) -> list[tuple[str, str]]:
        groups = selectedGroups if clickedGroup in selectedGroups else [clickedGroup]
        historyItems: list[tuple[str, str]] = []
        for group in groups:
            if not group:
                continue
            entity = self._entities.get(group)
            entityID = entity.id() if entity is not None else ''
            if entityID:
                historyItems.append((entityID, str(group)))
        return historyItems

    def showHistoryDialog(self, historyItems: list[tuple[str, str]]) -> None:
        if not historyItems:
            return
        entityID, entityName = historyItems[0]
        dialog = HistoryDialog(
            self._api,
            entityID,
            entityName,
            entityItems=historyItems,
            parent=self,
        )
        selectedGroups = self._getSelection()
        if dialog.exec() == QDialog.DialogCode.Accepted:
            rollbackResult = dialog.rollbackResult()
            if rollbackResult is not None:
                group, data = rollbackResult
                self.updateSingleEntity(group, data)
                if selectedGroups:
                    self._setSelection(selectedGroups)
            else:
                self.refreshData()
...
class TableWidget(DataWidget):
    ...
    ROWS_PER_BATCH = 25  # Build multiple rows per event-loop tick for faster loading
