/* ──────────────────────────────────────────────────────────────────────────
  ...
  * - [Current Date] - Added timeout handling and performance optimizations
...
const saveHiddenColumnsToPipelineSetting = async (cols: Set<string>) => {
  ...
  const backendRowsPerPage = shouldFetchAllForPivot ? 1000 : pageProps.rowsPerPage;
  
  // Safety limit
  const MAX_ALLOWED_ROWS = 1000;
  const safeRowsPerPage = Math.min(backendRowsPerPage, MAX_ALLOWED_ROWS);
  ...
  useEffect(() => {
    if (pivotError) {
      const errorMsg = pivotError.message.includes('timeout') || pivotError.message.includes('abort')
        ? 'Request timeout. The query is taking too long. Try reducing filters or contact support.'
        : `Failed to load asset data: ${pivotError.message}`;
      
      setFetchError(errorMsg);
      setSnackbarOpen(true);
      console.error('Pivot fetch error:', pivotError);
    } else {
      setFetchError(null);
    }
  }, [pivotError]);
  ...
  useEffect(() => {
    if (!assets && !groups && total === 0 && currentProject && !isPivotLoading && !pivotError) {
      setError('Failed to load asset data. Please check your authentication or try again.');
      setSnackbarOpen(true);
    }
  }, [assets, groups, total, currentProject, isPivotLoading, pivotError]);
  ...
  const filteredAssets = useMemo(() => {
    ...
    // Early return if no filters and no selected top groups
    if (!nameFilter && approvalFilters.length === 0 && workFilters.length === 0 && selectedTopGroups.length === 0) {
      return assets;
    }
    ...
    const base = assets.filter((asset: any) => {
      // 0) Top-group filter (fast check first)
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (duration > 1000) {
        console.warn(`Slow component lifecycle: ${Math.round(duration)}ms`);
        console.log(`Assets: ${assets?.length || 0}, Groups: ${groups?.length || 0}`);
        console.log(`Filters active: ${filtersActive}, Selected groups: ${selectedTopGroups.length}`);
      }
    };
  }, [assets, groups, filtersActive, selectedTopGroups]);

  // Handle retry for failed loads
  const handleRetryLoad = useCallback(() => {
    setFetchError(null);
    setError(null);
    setPageProps(initPageProps);
    // Force re-fetch by toggling forceFetch
    ...
    >
      ...
      >
        ...
        <Typography variant="body2" mt={2}>
          Loading asset data... {Math.round(pivotProgress)}%
        </Typography>
        {pivotProgress > 0 && pivotProgress < 100 && (
          <Box width="200px" mt={2}>
            <LinearProgress variant="determinate" value={pivotProgress} />
          </Box>
        )}
      </Box>
    </Box>
