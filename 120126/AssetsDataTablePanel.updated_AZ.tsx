...
/** ----------------------------------------------------------------
 * Group item sorting (A→Z / Z→A) BEFORE pagination (matches List view)
 * - Treat "-" / "—" / empty as null so they always go to the bottom.
 * ---------------------------------------------------------------- */
...
const sortGroupItemsAZ = (
  groups: any[],
  sortKey: string,
  sortDir: SortDir
) => {
  // SortDir may include "none" in this codebase; treat it as "asc".
  ...
  return (groups || []).map((g: any) => ({
    ...
    items: [...(g.items || [])].sort((a: any, b: any) => {
      ...
      // Empty values always at bottom (ASC & DESC)
...
const AssetsDataTablePanel: React.FC<RouteComponentProps> = () => {
  ...
  // Sort items inside each group BEFORE pagination (ASC: A→Z, DESC: Z→A)
