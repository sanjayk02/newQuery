...
const useStyles = makeStyles(() => ({
  ...
  // ✅ NEW: Footer container that mimics list view behavior
  footerWrapper: {
    position: 'sticky',
    bottom: 0,
    left: 0,
  ...
  // Force pagination to right even if it has center alignment
  forceRight: {
...
const AssetsGroupedDataTable: React.FC<Props> = ({
  ...
  return (
    <Box className={classes.root}>
      {/* Main table content with scroll */}
      ...
      {/* ✅ FIXED: Footer that stays at bottom-right ALWAYS */}
      {tableFooter && (
        <Box className={classes.footerWrapper}>
          <Box className={classes.footerContent}>
            {/* Force right alignment even if tableFooter has center styling */}
