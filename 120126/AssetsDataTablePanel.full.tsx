...
const AssetsDataTablePanel: React.FC<RouteComponentProps> = () => {
  ...
  const filteredGroups = useMemo(() => {
    ...
    // 3) ✅ Sort items INSIDE each group (A→Z / Z→A) BEFORE pagination
    //    (Do NOT sort group headers; only items)
    const isEmpty = (v: any) =>
      v == null || v === '' || v === '-' || v === '—';

    const getItemSortValue = (item: any, key: string) => {
      if (!key) return '';
      // NAME column (group view uses group_1_name)
      if (key === 'group_1' || key === 'group_1_name') {
        return (
          item.group_1 ||
          item.group_1_name ||
          item.name ||
      ...
      if (key === 'relation') return item.relation || '';

      // Phase columns: mdl_work / mdl_appr / mdl_submitted
      const mm = key.match(/^(mdl|rig|bld|dsn|ldv)_(work|appr|submitted)$/i);
      if (mm) {
        const phaseKey = mm[1].toLowerCase();
        const kind = mm[2].toLowerCase();
        if (kind === 'submitted') {
          return (
            item[`${phaseKey}_submitted_at_utc`] ||
            item[`${phaseKey}_submitted_at`] ||
            ''
          );
        }
        if (kind === 'work') return item[`${phaseKey}_work_status`] || '';
        return item[`${phaseKey}_approval_status`] || '';
      }
    ...
    const compare = (a: any, b: any) => {
      const av = getItemSortValue(a, uiSortKey);
      const bv = getItemSortValue(b, uiSortKey);
      ...
      return uiSortDir === 'asc' ? cmp : -cmp;
    };

    const sorted = (filtered || []).map((g: any) => ({
      ...g,
      items: Array.isArray(g.items) ? [...g.items].sort(compare) : [],
    }));
