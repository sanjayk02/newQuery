...
const AssetsDataTablePanel: React.FC<RouteComponentProps> = () => {
  ...
  const filteredAssets = useMemo(() => {
    ...
    const base = assets.filter((asset: any) => {
      // 0) Top-group filter (Asset Groups)
      // If any top groups are selected, restrict results to those groups only.
      // Backend may return null/empty/"UNASSIGNED" for unassigned.
      if (selectedTopGroups.length > 0) {
        const normalizeTopGroup = (v: any) => {
          const s = (v || '').toString().trim();
          if (!s) return 'unassigned';
          const ls = s.toLowerCase();
          // Normalize common "unassigned" variants (null/empty/UNASSIGNED/etc.)
  ...
  const filteredGroups = useMemo(() => {
    ...
    const itemMatches = (item: any) => {
      // Asset name filter
      if (nameFilter) {
        // Backend payloads differ between list/grouped modes.
        // Support the common name keys so the search works in Group view as well.
        const n = (
          item.group_1 ||
          item.group_1_name ||
          item.name ||
  ...
  const filteredTotal = React.useMemo(() => {
    ...
    return (sortedGroupsForPaging || []).reduce((acc: number, g: any) => {
      const items = Array.isArray(g.items) ? g.items : [];
  ...
  // Sort items INSIDE each group BEFORE we paginate across groups/items.
  // This ensures sorting is consistent across pages (page 2 continues the sorted order of page 1).
  const sortedGroupsForPaging = React.useMemo(() => {
    const list = filteredGroups || [];

    // helper: treat placeholders as empty and always push them to the bottom
    const isEmpty = (v: any) => {
    ...
    const getValue = (item: any, key: string) => {
      ...
      // Submitted columns: prefer the actual timestamp field if available
      // key is like "mdl_submitted" -> try "mdl_submitted_at_utc"
      const m = key.match(/^(mdl|rig|bld|dsn|ldv)_submitted$/i);
      if (m) {
        const phase = m[1].toLowerCase();
        return item[`${phase}_submitted_at_utc`] ?? item[key];
      }
    ...
    const cmp = (a: any, b: any) => {
      const va = getValue(a, uiSortKey);
      const vb = getValue(b, uiSortKey);

      const ea = isEmpty(va);
      const eb = isEmpty(vb);
      if (ea && eb) return 0;
      if (ea) return 1;   // empty always bottom
      if (eb) return -1;
      ...
      if (typeof ca === "number" && typeof cb === "number") {
        return uiSortDir === "asc" ? ca - cb : cb - ca;
      }

      const sa = String(ca);
      const sb = String(cb);
      return uiSortDir === "asc" ? sa.localeCompare(sb) : sb.localeCompare(sa);
    };

    // Keep group order as-is; only sort items inside each group.
    return list.map((g: any) => {
      const items = Array.isArray(g.items) ? g.items : [];
      const sorted = [...items].sort(cmp);
      return { ...g, items: sorted };
    });
  }, [filteredGroups, uiSortKey, uiSortDir]);


const pagedGroups = React.useMemo(() => {
    const pg = paginateGroups(
      sortedGroupsForPaging || [],
      pageProps.page,
      pageProps.rowsPerPage
    );
