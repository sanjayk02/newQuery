...
/** -------------------------------------------------------------
 * Group-view sorting: sort ONLY items inside each group.
 * NOTE: some displayed fields (like *_SUBMITTED AT) come from
 * `${phase}_submitted_at_utc`, not from the column id directly.
 * ------------------------------------------------------------- */
function getSortValueForColumn(asset: any, col: Column) {
  if (!asset || !col) return null;

  if (col.id === "group_1_name") return asset.group_1_name ?? asset.group_1 ?? asset.name ?? null;
  if (col.id === "relation") return asset.relation ?? null;

  // Phase columns
  if (col.phase) {
    if (col.kind === "submitted") {
      return (
        asset[`${col.phase}_submitted_at_utc`] ??
        asset[`${col.phase}_submitted_at`] ??
        asset[`${col.phase}_submitted`] ??
        null
      );
    }
    // work/appr (and any other kinds) are stored as `${phase}_${kind}`
    if (col.kind) return asset[`${col.phase}_${col.kind}`] ?? null;
  }

  // Fallback: use the column id
  return asset[col.id] ?? null;
}

function normalizeSortValue(v: any) {
...
function compareAssetsByColumn(a: any, b: any, col: Column, dir: SortDir) {
  const av = normalizeSortValue(getSortValueForColumn(a, col));
  const bv = normalizeSortValue(getSortValueForColumn(b, col));

  // push nulls to bottom in both directions
                ...
                <React.Fragment key={groupName}>
                  ...
                  {!isCollapsed &&
                    (() => {
                      ...
                      const sortedItems = col && col.sortable ? [...items].sort((a, b) => compareAssetsByColumn(a, b, col, sortDir)) : items;
                      return sortedItems.map((asset: any, idx: number) => (
