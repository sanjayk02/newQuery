  ...
  if (col.id === "group_1_name") {
    return (
      <Typography noWrap style={{ fontSize: 12, fontWeight: 500 }}>
        {asset.group_1 || asset.group_1_name || ""}
      </Typography>
    );
  }
...
// ------------------------------
// ✅ Sorting (Group View)
// - Sort ONLY items within a group
// - Keep group header order as-is
// - Treat '-', '—', '' as empty and ALWAYS push to bottom (asc + desc)
// - Status columns use a sensible rank before fallback string compare
// ------------------------------

function isEmptySortValue(v: any) {
  if (v == null) return true;
  const s = String(v).trim();
  return s === "" || s === "-" || s === "—";
}

function statusRank(v: any): number | null {
  if (isEmptySortValue(v)) return null;
  const s = String(v).toLowerCase();
  // Higher = "bigger". We still respect asc/desc; this just defines priority.
  if (s.includes("approved")) return 600;
  if (s.includes("review")) return 500;
  if (s === "check") return 400;
  if (s.includes("hold")) return 300;
  if (s.includes("retake")) return 200;
  if (s.includes("tbd")) return 150;
  if (s.includes("omit")) return 100;
  return 50;
}

function getSortValue(asset: any, col: Column): any {
  if (!asset) return null;

  // fixed columns
  if (col.id === "group_1_name") return asset.group_1 || asset.group_1_name || "";
  if (col.id === "relation") return asset.relation || "";
  ...
  if (col.kind === "submitted") {
    // IMPORTANT: UI renders from `${phase}_submitted_at_utc`
    return asset[phase + "_submitted_at_utc"] || "";
  }

  const field = col.kind === "work" ? "work_status" : "approval_status";
  return asset[phase + "_" + field] || "";
}

function compareWithinGroup(a: any, b: any, sortKey: string, sortDir: SortDir): number {
  const col = COLUMNS.find((c) => c.id === (sortKey as ColumnId));
  if (!col) return 0;

  const avRaw = getSortValue(a, col);
  const bvRaw = getSortValue(b, col);

  const aEmpty = isEmptySortValue(avRaw);
  const bEmpty = isEmptySortValue(bvRaw);
  // ✅ empty always bottom
  if (aEmpty && bEmpty) return 0;
  if (aEmpty) return 1;
  if (bEmpty) return -1;

  // Dates: submitted_at_utc (or anything parseable)
  if (col.kind === "submitted") {
    const at = Date.parse(String(avRaw));
    const bt = Date.parse(String(bvRaw));
    if (!Number.isNaN(at) && !Number.isNaN(bt)) {
      return sortDir === "asc" ? at - bt : bt - at;
    }
  }

  // Status rank for work/appr columns
  if (col.kind === "work" || col.kind === "appr") {
    const ar = statusRank(avRaw);
    const br = statusRank(bvRaw);
  ...
  const as = String(avRaw).toLowerCase();
  const bs = String(bvRaw).toLowerCase();
        ...
        <Table size="small" stickyHeader className={classes.table}>
          ...
          <TableBody>
            {groups.map((group) => {
              ...
              // totalCount is attached in AssetsDataTablePanel (ShotGrid-style: visible of total)
              ...
              return (
                <React.Fragment key={groupName}>
                  ...
                  {!isCollapsed &&
                    [...(group.items || [])]
                      .sort((a: any, b: any) => compareWithinGroup(a, b, sortKey, sortDir))
