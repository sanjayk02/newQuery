...
/** ----------------------------------------------------------------
 * Group item sorting (A→Z / Z→A) BEFORE pagination
 * - Keeps group header order as-is
 * - Sorts only items inside each group
 * - Treats null/undefined/"-"/"—"/"" as empty and pushes to bottom
 * ---------------------------------------------------------------- */
const normalizeSortValue = (v: any): string | null => {
  if (v == null) return null;
  if (v === '-' || v === '—' || v === '') return null;
  return String(v).toLowerCase();
};

const sortGroupItemsAZ = (groups: any[], sortKey: string, sortDir: 'asc' | 'desc'): any[] => {
  return (groups || []).map((g) => {
    const items = Array.isArray(g.items) ? [...g.items] : [];
    items.sort((a: any, b: any) => {
      ...
      // empty values always bottom (both ASC and DESC)
...
const AssetsDataTablePanel: React.FC<RouteComponentProps> = () => {
  ...
  const filteredTotal = React.useMemo(() => {
    ...
    return (sortedFilteredGroups || []).reduce((acc: number, g: any) => {
      const items = Array.isArray(g.items) ? g.items : [];
   ...
   */
  // Sort items within each group BEFORE paginating (keeps page index, matches list-view A→Z / Z→A)
  const sortedFilteredGroups = React.useMemo(() => {
    if (!isGrouped) return filteredGroups || [];
    return sortGroupItemsAZ(filteredGroups || [], sortKey, sortDir);
  }, [filteredGroups, isGrouped, sortKey, sortDir]);
