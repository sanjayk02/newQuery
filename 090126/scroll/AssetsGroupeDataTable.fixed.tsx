...
/**
 * ✅ Scroll behavior (match List view)
 *
 * The parent panel (`AssetsDataTablePanel -> StyledPaper`) already provides the
 * scroll container via `overflow: auto`.
 *
 * If we create another internal `overflow: auto` container here, we end up with
 * *nested scroll containers* (Panel scroll + Group scroll). That breaks sticky
 * footer/header behavior and can make the scrollbar feel "missing".
 *
 * So this component should NOT create its own scroller; it should simply render
 * the table and let the parent panel handle scrolling.
 */
const useStyles = makeStyles(() => ({
  ...
  // parent panel provides scrolling; keep this wrapper non-scroll
  scroller: {
    flex: 1,
    minHeight: 0,
    overflow: "visible",
