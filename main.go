...
func main() {
	...
	// GET /api/latest/review-submissions/:project?limit=50&offset=0
	router.GET("/api/latest/review-submissions/:project", func(c *gin.Context) {
		...
		// 2. Call the Repository method (accessing gormDB and reviewInfoRepository from main's scope)
		// This line is now correct, relying on the variable initialization later in the function.
	...
	apiRouter := router.Group("/api")
	{
		...
		// Review API
        // This is the CRITICAL initialization line accessed by the router.GET handler above.
		...
		// --- Simple debug endpoint: prints to server log and returns 200 ---
		apiRouter.GET("/debug/print", func(c *gin.Context) {
			log.Println("👉 debug/print hit: hello from main.go")
			c.String(http.StatusOK, "ok")
		})
