...
// REMOVE useFetchAssetReviewInfos
...
// Updated columns to include sortable keys matching backend pivotSorters
const columns: Column[] = [
  ...
  },
  {
    ...
    label: 'Name',
    sortable: true,
    sortKey: 'group_1', // Match backend ListOrderedAssets key
  },
  {
    id: 'mdl_work_status',
    label: 'MDL WORK',
    ...
    sortable: true,
    sortKey: 'mdl_work', // Match backend pivotSorters key
  },
  {
...
// ... (MultiLineTooltipTableCell component remains unchanged)
...
const AssetRow: React.FC<AssetRowProps> = ({
  ...
  const getPhaseData = (phase: string) => {
    ...
    const workStatus: Status | undefined = workStatusValue ? WORK_STATUS[workStatusValue.toLowerCase()] : undefined;
    const approvalStatus: Status | undefined = approvalStatusValue ? APPROVAL_STATUS[approvalStatusValue.toLowerCase()] : undefined;
    
    const submittedAt = submittedAtValue ? new Date(submittedAtValue) : null;
...
const AssetsDataTable: React.FC<AssetsDataTableProps> = ({
  ...
  const thumbnailAssets = assets.map(a => ({ name: a.group_1, relation: a.relation }));
  const { thumbnails } = useFetchAssetThumbnails(project, thumbnailAssets); // REMOVED useFetchAssetReviewInfos
