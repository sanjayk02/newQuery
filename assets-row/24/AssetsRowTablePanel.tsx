...
const GROUP_BORDER = 'rgba(255,255,255,0.28)';      // strong border color
const GROUP_BORDER_SOFT = 'rgba(255,255,255,0.16)'; // subtle inner top/bottom lines
...
// NOTE: no optional chaining here
const getWorkflowBoxStyle = (
  ...
  const topOn = !!(opts && opts.top);
  const bottomOn = !!(opts && opts.bottom);
  ...
  return {
    ...
    borderTop: topOn ? `1px solid ${GROUP_BORDER_SOFT}` : undefined,
    borderBottom: bottomOn ? `1px solid ${GROUP_BORDER_SOFT}` : undefined,
  };
};
                      ...
                      {HEADER_COLUMNS.map((col) => {
                        ...
                        return (
                          <DataCell
                            key={col.id}
                            style={{
                              ...getWorkflowBoxStyle(col.id, { top: true, bottom: true }),
