...
const AssetsDataTablePanel: React.FC<RouteComponentProps> = () => {
  ...
  /**
   * Search box text (UI).
   * We debounce commits to the backend/filterProps to avoid re-fetching on every keystroke,
   * while still filtering already-fetched rows immediately.
   */
  ...
  // Debounce the search term so we don't re-fetch on every keystroke.
  useEffect(() => {
    const t = window.setTimeout(() => {
      setDebouncedSearch((barSearch || '').trim());
    }, 250);
    return () => window.clearTimeout(t);
  }, [barSearch]);

  // Commit the debounced search into the shared filterProps (used by other UI bits + backend call).
  useEffect(() => {
    setFilterProps((prev) => ({ ...prev, assetNameKey: debouncedSearch } as any));
  }, [debouncedSearch]);
  ...
  const effectivePhase = useMemo(() => {
    const nameFilterActive = !!(barSearch || '').trim();
  ...
  const filtersActive = useMemo(() => {
    const nameActive = !!(barSearch || '').trim();
    return (
      nameActive ||
  ...
  const { assets, total, groups } = useFetchAssetsPivot(
    ...
    sortDir,
    effectivePhase,
    // pass the debounced term to the backend
    debouncedSearch,
    approvalArray,
    workArray,
    viewMode,
  ...
  const filteredAssets = useMemo(() => {
    ...
    const nameFilter      = (barSearch || '').trim().toLowerCase();
    const nameTokens      = nameFilter.split(/\s+/).filter(Boolean);
    ...
    const base = assets.filter((asset: any) => {
      ...
      if (nameTokens.length > 0) {
        const name = (asset.group_1 || asset.group_1_name || '').toString().toLowerCase();
        const rel  = (asset.relation || '').toString().toLowerCase();
        const hay  = `${name} ${rel}`;
        // AND-match all tokens (more precise than single substring)
  ...
  const filteredGroups = useMemo(() => {
    ...
    const itemMatches = (item: any) => {
      ...
      if (nameTokens.length > 0) {
        ...
        const rel = (item.relation || '').toString().toLowerCase();
        const hay = `${n} ${rel}`;
        if (!nameTokens.every((t) => hay.includes(t))) return false;
      }
    ...
    const anyInnerFilter =
      nameTokens.length > 0 || approvalFilters.length > 0 || workFilters.length > 0;
    ...
    <StyledContainer maxWidth={false}>
      <AssetTableFilter
        ...
        headerRight={(
          ...
          setBarSearch(v);
          // reset to first page immediately; fetch/filter will follow via debounce
