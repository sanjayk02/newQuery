...
/**
 * Map grouped-table column ids to the canonical column ids used by:
 * - Column visibility (hiddenColumns set)
 * - Column drawer (AssetDataTableFilter)
 *
 * NOTE: Sorting still uses grouped ids like `mdl_work`, `mdl_appr`, `mdl_submitted`
 * because the Panel expects those UI sort keys in `resolveServerSort`.
 */
function hideKeyForColumn(id: ColumnId): string {
  // fixed
  if (id === "thumbnail") return "thumbnail";
  if (id === "group_1_name") return "group_1_name";
  if (id === "relation") return "relation";

  // phase columns
  const m = String(id).match(/^(mdl|rig|bld|dsn|ldv)_(work|appr|submitted)$/);
  if (!m) return String(id);

  const phase = m[1];
  const kind = m[2];

  if (kind === "work") return `${phase}_work_status`;
  if (kind === "appr") return `${phase}_approval_status`;
  return `${phase}_submitted_at`;
}

function sortKeyForColumn(id: ColumnId): string {
  // Align grouped-table sorting keys with Panel's resolveServerSort()
  if (id === "group_1_name") return "group_1";
  return String(id);
}
...
const AssetsGroupedDataTable: React.FC<Props> = ({
  ...
  const visibleColumns = useMemo(
    () => COLUMNS.filter((c) => !hiddenColumns.has(hideKeyForColumn(c.id))),
    [hiddenColumns]
  );
            ...
            <TableRow>
              {visibleColumns.map((col) => (
                <TableCell
                  ...
                  onClick={() => col.sortable && onSortChange(sortKeyForColumn(col.id))}
                  style={{
                ...
                >
                  ...
                  >
                    ...
                    {sortKey === sortKeyForColumn(col.id) &&
                      (sortDir === "asc" ? (
