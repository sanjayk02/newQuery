/* ──────────────────────────────────────────────────────────────────────────
    ...
    * - [TODAY] - Optimized queries for performance, fixed timeout issues
...
func (r *ReviewInfo) CountLatestSubmissions(
    ...
    log.Printf("[DEBUG] CountLatestSubmissions called for project: %s", project)
    start := time.Now()
    defer func() {
        log.Printf("[DEBUG] CountLatestSubmissions completed in %v", time.Since(start))
    }()
    ...
    query := `
        SELECT COUNT(DISTINCT CONCAT(group_1, '|', relation)) as total_count
        FROM t_review_info
        WHERE project = ? 
          AND root = ? 
          AND deleted = 0
...
func (r *ReviewInfo) ListLatestSubmissionsDynamic(
    ...
    log.Printf("[DEBUG] ListLatestSubmissionsDynamic called for project: %s, limit: %d, offset: %d",
        project, limit, offset)
    start := time.Now()
    defer func() {
        log.Printf("[DEBUG] ListLatestSubmissionsDynamic completed in %v", time.Since(start))
    }()
    ...
    // ENFORCE LIMIT: Never return more than 100 rows
    if limit > 100 {
        limit = 100
        log.Printf("[WARN] ListLatestSubmissionsDynamic: limit capped at 100")
    }

    db := r.db.WithContext(ctx)

    // OPTIMIZED QUERY - Simpler, no CTE, uses subquery instead of window function
    query := `
        SELECT 
            t1.root,
            t1.project,
        ...
        WHERE t1.project = ? 
          AND t1.root = ? 
          AND t1.deleted = 0
          AND t1.modified_at_utc = (
              SELECT MAX(t2.modified_at_utc)
              FROM t_review_info t2
              WHERE t2.project = t1.project
                AND t2.root = t1.root
                AND t2.group_1 = t1.group_1
                AND t2.relation = t1.relation
                AND t2.deleted = 0
          )
    ...
    args := []any{project, root, limit, offset}

    var rows []LatestSubmissionRow
    if err := db.Raw(query, args...).Scan(&rows).Error; err != nil {
        log.Printf("[ERROR] ListLatestSubmissionsDynamic query failed: %v", err)
        ...
        if err := db.Raw(fallbackQuery, args...).Scan(&rows).Error; err != nil {
            return nil, fmt.Errorf("ListLatestSubmissionsDynamic: %w", err)
        }
    }

    log.Printf("[DEBUG] ListLatestSubmissionsDynamic returned %d rows", len(rows))
    return rows, nil
}
    ...
    query := fmt.Sprintf(`
        WITH latest_per_asset AS (
            SELECT 
                ...
                t1.modified_at_utc,
                COALESCE(
                    JSON_UNQUOTE(JSON_EXTRACT(t1.\` + "`groups`" + `, '$[0]')),
                    t1.group_1
                ) as leaf_group_name,
                ROW_NUMBER() OVER (
        ...
        LEFT JOIN t_group_category_group AS gcg
            ON gcg.project = lpa.project
            AND gcg.deleted = 0
            AND gcg.path = lpa.leaf_group_name
