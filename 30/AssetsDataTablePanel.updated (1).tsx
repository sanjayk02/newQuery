...
import { Asset, FilterProps, PageProps, PivotGroup, SortDir } from './types'; // Added SortDir import
...
const AssetsDataTablePanel: React.FC<RouteComponentProps> = () => {
  ...
  /* Backend pagination adjustments
     - When filters are active we fetch a big chunk (client paginates).
     - When GROUPED view is active we ALSO fetch a big chunk so a single group isn't split by the server
       (e.g. "CHARACTER (15)" but only 8 visible). Grouped paging is handled client-side.
  */
  const backendPage = (filtersActive || isGrouped) ? 0 : pageProps.page;
  const backendRowsPerPage = (filtersActive || isGrouped) ? 5000 : pageProps.rowsPerPage;
...
// Build groups client-side for ShotGrid-like grouped view.
// NOTE: In grouped view we fetch a large page from backend (see backendRowsPerPage) and group locally
// to avoid server paging splitting a group (e.g. CHARACTER total 15 but only 8 shown).
const groupedFromAssets = useMemo(() => {
  if (!isGrouped) return groups;

  const by = new Map<string, { top_group_node: string | null; items: Asset[]; total_count: number }>();

  for (const a of assets) {
    const raw = (a as any).top_group_node || 'UNASSIGNED';
    const key = String(raw).trim() || 'UNASSIGNED';
    const existing = by.get(key);
    if (existing) {
      existing.items.push(a);
      existing.total_count += 1;
    } else {
      by.set(key, { top_group_node: key, items: [a], total_count: 1 });
    }
  }

  const normalize = (v: string | null | undefined) => {
    const s = String(v || '').trim();
    return s === '' ? 'unassigned' : s.toLowerCase();
  };

  const ordered = Array.from(by.values()).sort((a, b) => {
    const na = normalize(a.top_group_node);
    const nb = normalize(b.top_group_node);
  ...
  return ordered as unknown as PivotGroup[];
}, [isGrouped, groups, assets]);
  ...
  const tableFooter = isGrouped ? null : (
            ...
            {isGrouped ? (
              <Box width="100%" display="flex" flexDirection="column">
                <AssetsGroupedDataTable
                  groups={groupedFromAssets}
                  page={pageProps.page}
                  rowsPerPage={pageProps.rowsPerPage}
                  // Use UI sort state so grouped view matches the sort arrow
                  sortKey={uiSortKey}
                  sortDir={uiSortDir}
                />
                {tableFooter}
