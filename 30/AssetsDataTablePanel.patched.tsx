...
const AssetsDataTablePanel: React.FC<RouteComponentProps> = () => {
  ...
  const handleSortChange = (newUiKey: string) => {
    // In GROUPED view we keep the server ordering stable (so the "top group nodes"
    // don't reshuffle across pages). We only sort *within* each group on the client.
    if (isGrouped) {
      const nextUiDir: SortDir =
        uiSortKey === newUiKey ? (uiSortDir === 'asc' ? 'desc' : 'asc') : 'asc';
      setUiSortKey(newUiKey);
      setUiSortDir(nextUiDir);
      return;
    }
