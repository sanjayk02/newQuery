 ...
 * - persistHiddenColumns: Saves hidden column settings to both localStorage and pipeline settings.
...
// Component index mapping for component columns
...
// Phase mapping for component columns
const componentPhaseMap: { [key: string]: string } = {
  'mdlRend': 'mdl',
  'bldAnm': 'bld',
  'bldRend': 'bld',
  'ldvMdl': 'ldv'
...
// Helper function to find component data for an asset
const findComponentData = (componentList: any[], componentKey: string) => {
  if (!componentList || componentList.length === 0) return null;
  
  // Strategy 1: Match by component name (if available)
  let item = componentList.find((item: any) => 
    item.component && item.component.toLowerCase() === componentKey.toLowerCase()
  );
  
  // Strategy 2: Match by phase in latest_document
  if (!item) {
    const expectedPhase = componentPhaseMap[componentKey];
    item = componentList.find((item: any) => 
      item.latest_document?.phase?.toLowerCase() === expectedPhase
    );
  }
  
  // Strategy 3: Use index mapping as fallback
  if (!item) {
    const index = componentIndexMap[componentKey];
    item = componentList[index];
  }
...
/*
===============================================================================
 * Assets Data Table Panel
 * This component is responsible for displaying and managing the asset data table
 * within a project context. It includes state management, filtering, sorting,
 * and column visibility handling.
===============================================================================
*/
const AssetsDataTablePanel: React.FC<RouteComponentProps> = () => {
  ...
  // Debug latestComponents
  useEffect(() => {
    if (Object.keys(latestComponents).length > 0) {
      console.log('🔍 DEBUG - LatestComponents structure:');
      Object.entries(latestComponents).forEach(([key, value]) => {
        console.log(`Asset: ${key}`);
        console.log('  Full data:', JSON.stringify(value, null, 2));
        (value as any[]).forEach((item: any, index: number) => {
          console.log(`  [${index}] component:`, item.component);
          console.log(`  [${index}] latest_document:`, item.latest_document);
        });
      });
    }
  }, [latestComponents]);
  ...
  // debug latestComponents
  console.log('AssetsDataTablePanel - Data ----->>', {assets: rawAssets, phaseComponents, latestComponents });
  ...
  const resolveServerSort = (key: string): { sort: string; phase: string } => {
    ...
    // Add component handling to the regex and logic
    if (key === 'component' || key === 'component_only') { 
      return { sort: 'component', phase: 'none' };
    }

    // Component columns special handling - these might need client-side sorting
...
const saveHiddenColumnsToPipelineSetting = async (cols: Set<string>) => {
  ...
  /* 
  ===============================================================================
  # Client-side sorting of items within groups based on current UI sort key/dir.
  # This is applied on top of the server-side sorting, allowing for dynamic sorting
  # of the currently loaded page of items without refetching from the server.
  ===============================================================================
  */
  ...
  }, [sortedGroups, uiSortKey, uiSortDir, latestComponents, enhancedCompareTake, enhancedCompareStrings, enhancedCompareDates]);

  /*
  ===============================================================================
  # Client-side pagination of groups and their items.
  # This is applied after all filtering and sorting, allowing for dynamic pagination
  # of the currently loaded groups without refetching from the server.
  ===============================================================================
  */
  ...
  */
  function paginateGroups(
    ...
    /*
    ===============================================================================
    # Apply pagination to the flattened list of items.
    # This ensures that we are paginating the actual items across groups, rather than
    # paginating groups themselves which could lead to uneven distribution of items.
    =============================================================================
    */
...
/*
===============================================================================
# Column metadata definition for the assets data table.
# This defines the columns that can be displayed in the table, along with their labels.
# The 'id' field corresponds to the key in the asset data, and 'label' is the human-readable name.
===============================================================================
*/
const COLUMN_META: { id: string; label: string }[] = [
  ...
  { id: "mdlRend", label: "MDLREND Submitted "},
  ...
  { id: "bldAnm", label: "BLDANM Submitted "},
  { id: "bldRend", label: "BLDREND Submitted "},
  ...
  { id: "ldvMDL", label: "LDVMDL Submitted "},
